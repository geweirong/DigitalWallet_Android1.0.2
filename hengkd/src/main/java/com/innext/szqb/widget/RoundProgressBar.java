...
public class RoundProgressBar extends View {
	...
	public RoundProgressBar(Context context, AttributeSet attrs, int defStyle) {
		...
		roundColor = mTypedArray.getColor(
				...
				Color.argb(100, 0x99, 0x99, 0x99));
		roundProgressColor = mTypedArray.getColor(
				R.styleable.RoundProgressBar_roundProgressColor, getResources().getColor(R.color.subject_color));
		textColor = mTypedArray.getColor(
				R.styleable.RoundProgressBar_roundtextColor, getResources().getColor(R.color.subject_color));
		textSize = mTypedArray.getDimension(
				R.styleable.RoundProgressBar_roundtextColor, 16);
	...
	protected void onDraw(final Canvas canvas) {
		...
		if (percent == 100) {
//			paint.setColor(Color.argb(100, 0xbb, 0xbb, 0xbb));
			paint.setColor(getResources().getColor(R.color.subject_color));
		} else {
			paint.setColor(textColor);
		}
		paint.setTextSize(App.mApp.dip2px(textSize));
		...
		paint.setStrokeWidth(roundWidth); // 设置圆环的宽度
		if (percent == 100) {
//			paint.setColor(Color.argb(100, 0x99, 0x99, 0x99));
			paint.setColor(getResources().getColor(R.color.subject_color));
		} else {
...
//						mHandler1.sendEmptyMessageDelayed(INTERVAL, 250/progress);
//					}
					mHandler1.sendEmptyMessageDelayed(INTERVAL, 10);
				} else {
					setProgress(curProgress);
				}
				break;
